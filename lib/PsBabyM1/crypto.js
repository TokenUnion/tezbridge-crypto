...
const [genRandomBytes, deriveKeyByPBKDF2] = (() => {
  if (process.env.BROWSER_OPT) {
    return [
      (len         ) => window.crypto.getRandomValues(new Uint8Array(len)),
      (password         , salt             ) =>
...
export function blake2bHash(input             , len          = 32) {
  return blake.blake2b(input, null, len)
}

export function genMnemonic(strength          , lang          ) {
...
export class Key {
  ...
  constructor(name                                   , secret_key             , pub_key             ) {
    this.name = name
...
function getKeyFromEd25519(input             ) {
...
function getKeyFromSecp256k1(key             ) {
...
export async function decryptKey(encrypted         , password         )                {
...
export async function encryptKey(scheme                                   , 
                           unencrypted             , 
                           password         )                   {
...
export function getKeyFromSecretKey(sk        ) {
...
export function deriveKeyFromWords(words         , pwd          , path         , scheme                                   ) {
...
export function signOperation(input_operation                      , secret_key         ) {
